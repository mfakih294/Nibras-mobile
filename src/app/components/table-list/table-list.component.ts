...
//t: String;
    ...
    constructor(private storage:Storage,
        ...
        this.storage.forEach((value, key, index) => {
            if (key.startsWith('u-d')) {
this.updates[value] = true
            }
        });
        ...
        this.storage.forEach((value, key, index) => {
            if (key.startsWith('u-a-')) {
                console.log('found append in storage', value, ' with key '  + key)
                if (!this.appends[key.split('=')[0]])
                    this.appends[key.split('=')[0]] = value
                        else
                this.appends[key.split('=')[0]] += value;

                console.log('appends ' , this.appends)
            }
        })
...
//    this.storage.set('tosyncText', {})
    ...
    async openModal(ecode, module, resourceType, id, summary, language, priority, date, description, nbFiles, filesList, freshRecord) {
        ...
        //this.storage.get('r-' + id).then(val => {
            console.log('inside open modal using storage');
    ...
    const modal = await this.modalController.create({
            component: ExampleModalPage, cssClass: 'nbrModal',
        //animated: true, backdropDismiss: true, showBackdrop: true,
            componentProps: {
                "ecode": ecode,
                "module": module,
    ...
    //console.log('appends after creating modal ' , appends)
...
//})
    ...
    clean(text) {
        if (text)
            return (text != '' ? text.substring(0, 120) : '') //replace(/<br\/>/g, ' ')
        else return ''
    }
...
// Call as
    ...
    markDone(checked, ecode, id) {


        this.storage.set('u-d-' + ecode + '-' + id, 'u-d-' + ecode + '-' + id).then(val => {
            console.log('updated ', val);

            this.updates.unshift(val)
            console.log('updated ', this.updates);
        });
...
/*
        ...
        this.storage.get('tosyncText').then((val) => {
            ...
            if (val == {} || val == null || val == [] || Object.keys(val).length == 0) {
                //val = {['ecode': ecode, 'id': id]}
                console.log('val empty');
            }
            //else newval = JSON.parse(val)
            //else val.push(['ecode': ecode, 'id': id])
...
//     console.log('add to sync: ' , item.ecode + '' + item.id + '');
            ...
            console.log('add to sync: ', ecode + '' + id + '');
        })
